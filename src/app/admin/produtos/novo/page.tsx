...
export default function NovoProdutoPage() {
  ...
  const [tamanhosAtivos, setTamanhosAtivos] = useState<Record<string, boolean>>({
    PP: false, P: true, M: true, G: true, GG: false,
  });
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-8 text-2xl font-semibold text-neutral-900">Novo Produto</h1>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                {imagemPreview && (
                  <div className="mt-4">
                    <img src={imagemPreview} alt="Preview" className="h-32 w-32 object-cover rounded-xl border border-brand-200 shadow-sm" />
                  </div>
                )}
              </div>
            </div>
            ...
            {tamanhos.map((tam) => (
              <div key={tam} className="flex items-center gap-4">
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={tamanhosAtivos[tam]}
                    onChange={(e) => setTamanhosAtivos({ ...tamanhosAtivos, [tam]: e.target.checked })}
                ...
                {tamanhosAtivos[tam] && (
                  <input
                    ...
                    min="0"
                    value={estoques[tam]}
                    onChange={(e) => setEstoques({ ...estoques, [tam]: parseInt(e.target.value) || 0 })}
                    placeholder="Estoque"
                    className="w-24 rounded-xl border border-brand-200 bg-brand-50/30 px-3 py-1.5 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
                  />
                )}
              </div>
            ))}
